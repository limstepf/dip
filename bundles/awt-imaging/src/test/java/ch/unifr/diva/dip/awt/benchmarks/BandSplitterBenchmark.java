...
public class BandSplitterBenchmark {
	...
	public Result concurrent3xLoopOp(Resources r) {
		...
		Rectangle tile = Filter.getOptimalTileSize(r.numThreads, r.image);

		op.setBand(0);
		result.band1 = Filter.filter(Resources.dtp, op, r.image, null);

		op.setBand(1);
		result.band2 = Filter.filter(Resources.dtp, op, r.image, null);

		op.setBand(2);
		result.band3 = Filter.filter(Resources.dtp, op, r.image, null);
